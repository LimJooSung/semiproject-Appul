...
public class SelectingPresenterDAO {
	...
	public ArrayList<SelectingPresenterVO> getPresenterList() throws SQLException{
		...
		try{
			...
			sql.append("where m.id = s.id  and s.cnt_presentation=(select min(cnt_presentation) from selecting_presenter s, member m where m.getout='N' and m.id=s.id)");
	...
	public ArrayList<SelectingPresenterVO> getAllPresenterList() throws SQLException{
		...
		try{
			con=getConnection(); 
			StringBuilder sql=new StringBuilder();
			sql.append("select mem_name, cnt_presentation, mem_number from member m, selecting_presenter s where m.id=s.id and getout='N'");
			pstmt=con.prepareStatement(sql.toString());			
			rs=pstmt.executeQuery();	
			while(rs.next()){					
				String memberName = rs.getString("mem_name");
